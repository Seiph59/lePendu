...
void selectionMot(char *pointeurMotSecret)
{
    ...
    if (fichier != NULL)
    {
       while(fgets(ligneFichier,20,fichier) != NULL)        // Passe de ligne en ligne avec fgets tant qu'il y a des caractères sur la ligne suivante
       {
            //printf("%s", ligneFichier);   pour verifier si les mots étaient bien parcouru
            i++;
       }

        rewind(fichier);
         //printf("\nnombre de mots %d\n", i);
        nombreAleatoire = rand()%i;             // definit nombre aléatoire entre 0 et  i
